...
```bash
ENABLE_CAMERAS=1 python scripts/navigate_ros2.py \
  --config configs/navigate/ex001_nav.yaml
```

导航入口默认使用 CPU PhysX，以避免当前 Isaac Sim / Isaac Lab 组合在 CUDA
PhysX 回退时产生 `ProxyArray` 类型冲突。RTX 相机和 RTX lidar 仍使用 NVIDIA GPU
渲染。只有在确认本机 GPU PhysX 可用后，才显式传入 `--device cuda:0`。
