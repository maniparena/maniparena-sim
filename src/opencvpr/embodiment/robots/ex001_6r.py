"""EX001-6R embodiment config for the open-source runtime."""
...
def ex001_6r_left_eef_delta_pos(env, ee_frame_cfg=SceneEntityCfg("left_ee_frame")) -> torch.Tensor:
    pos, quat = _get_ee_world(env, ee_frame_cfg)
    ref_pos, _ = _get_or_init_ee_ref(env, "_left_ee_ref", pos, quat)
    return pos - ref_pos


def ex001_6r_left_eef_delta_quat(env, ee_frame_cfg=SceneEntityCfg("left_ee_frame")) -> torch.Tensor:
    pos, quat = _get_ee_world(env, ee_frame_cfg)
...
def ex001_6r_right_eef_delta_pos(env, ee_frame_cfg=SceneEntityCfg("right_ee_frame")) -> torch.Tensor:
    pos, quat = _get_ee_world(env, ee_frame_cfg)
    ref_pos, _ = _get_or_init_ee_ref(env, "_right_ee_ref", pos, quat)
    return pos - ref_pos


def ex001_6r_right_eef_delta_quat(env, ee_frame_cfg=SceneEntityCfg("right_ee_frame")) -> torch.Tensor:
    pos, quat = _get_ee_world(env, ee_frame_cfg)
...
@register_asset
class EX0016REmbodiment(EmbodimentBase):
    """EX001-6R dual-arm fixed-base embodiment."""
    ...
    class StateObservationsCfg:
        ...
        class PolicyCfg(ObsGroup):
            ...
            eef_delta_pos = ObsTerm(func=ex001_6r_left_eef_delta_pos, params={"ee_frame_cfg": SceneEntityCfg("left_ee_frame")})
            eef_delta_quat = ObsTerm(func=ex001_6r_left_eef_delta_quat, params={"ee_frame_cfg": SceneEntityCfg("left_ee_frame")})
            right_eef_delta_pos = ObsTerm(func=ex001_6r_right_eef_delta_pos, params={"ee_frame_cfg": SceneEntityCfg("right_ee_frame")})
            right_eef_delta_quat = ObsTerm(func=ex001_6r_right_eef_delta_quat, params={"ee_frame_cfg": SceneEntityCfg("right_ee_frame")})
    ...
    def __init__(
        ...
        env_pos = _parse_env_tuple(os.environ.get("ISAACLAB_ARENA_EX001ARM_XR_ANCHOR_POS"), 3)
        env_rot = _parse_env_tuple(os.environ.get("ISAACLAB_ARENA_EX001ARM_XR_ANCHOR_ROT"), 4)
        self.xr = XrCfg(
